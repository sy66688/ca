...
*/
let token = []
if (process.env.MyShopToken) {
  if (process.env.MyShopToken.includes('\n')) {
    token = [...process.env.MyShopToken.split('\n'),...token]
  } else {
    token = [...process.env.MyShopToken.split('&'),...token]
  }
}
if (!token.length) {
  console.log('无店铺签到token,不执行.需自备token:环境变MyShopToken: tk1&tk2.')
  return
}
const $ = new Env('店铺签到');
